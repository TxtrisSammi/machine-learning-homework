...
\begin{enumerate}
  ...
  \item Was \texttt{np.argmax([1, 1]) = np.argmax([(1-0.55)/0.45, 1])}?
        ...
        % solution goes here
	\begin{sol}
		They should be equal, however due to how floating point
		arithmatic works, the computer does not see $(1-0.55)/0.45$ as
		equal to 1.
	\end{sol}
\end{enumerate}
...
\begin{enumerate}
  ...
  \item Use the data to find out what percentage of men survived on
        ...
        % solution goes here
        \begin{sol}
          11.6\% of men and 28.3\% of women survived the Titanic (using the training set)
        \end{sol}
  \item Finish the
        ...
        % solution goes here
	\begin{sol}
		0.77 (I somehow got 0.79 before, but I accidentally deleted the file and now I can't recover the extra 0.02 accuracy from before)
	\end{sol}
